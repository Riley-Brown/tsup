...
var generateExternal = async (external) => {
  ...
  for (const item of external) {
    ...
    const deps = await _chunk4SUVYI7Ujs.getProductionDeps.call(void 0, pkgPath);
    result.push(...deps);
  }
  return result;
};
async function runEsbuild(options, {
  ...
  if (result && result.warnings && !_chunk4SUVYI7Ujs.getSilent.call(void 0, )) {
...
// src/api-extractor.ts
var logger2 = _chunk4SUVYI7Ujs.createLogger.call(void 0, );
...
async function build(_options) {
  const config = _options.config === false ? {} : await _chunk4SUVYI7Ujs.loadTsupConfig.call(void 0, 
    process.cwd(),
  ...
  await Promise.all(
    [...Array.isArray(configData) ? configData : [configData]].map(
      async (item) => {
        const logger3 = _chunk4SUVYI7Ujs.createLogger.call(void 0, _optionalChain([item, 'optionalAccess', _31 => _31.name]));
        ...
        const mainTasks = async () => {
          if (!_optionalChain([options, 'access', _33 => _33.dts, 'optionalAccess', _34 => _34.only])) {
            ...
            let depsHash = await _chunk4SUVYI7Ujs.getAllDepsHash.call(void 0, process.cwd());
            ...
            const buildAll = async () => {
              ...
              if (options.onSuccess) {
                if (typeof options.onSuccess === "function") {
                  ...
                  _optionalChain([onSuccessProcess, 'optionalAccess', _35 => _35.on, 'call', _36 => _36("exit", (code) => {
                    if (code && code !== 0) {
              ...
              watcher.on("all", async (type, file) => {
                ...
                if (options.watch === true) {
                  if (file === "package.json" && !buildDependencies.has(file)) {
                    const currentHash = await _chunk4SUVYI7Ujs.getAllDepsHash.call(void 0, process.cwd());
